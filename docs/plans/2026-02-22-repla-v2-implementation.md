# Repla V2 — Full Platform Expansion Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Transform the MVP into a feature-complete Repla clone with dual navbars, pure black theme, full-screen AI Copilot, functional floating trade widget, positions/orders panel, and symbols search.
...
```css
:root {
  /* ─── Core Background (Pure Black — matches Repla) ─── */
  --bg: #000000;
  --bg-raised: #0a0a0a;
  --bg-surface: #0f0f0f;
  --bg-overlay: #141414;
  ...
  /* ─── Borders (Repla uses ultra-thin 0.667px) ─── */
...
Key changes: `--bg` → `#000000`, `--buy` → `#22ab94` (Repla teal), `--sell` → `#f23645` (Repla red), added `--link`, `--border-panel`, `--radius-pill`.
...
```css
/* ─── Nav Pill Button (Repla navbar1 style) ─── */
.nav-pill {
  display: flex;
  align-items: center;
  gap: 6px;
...
/* ─── Panel Separator (ultra-thin Repla style) ─── */
.panel-border-top {
...
git commit -m "feat: update theme to pure black + add Repla utility classes"
```
...
```bash
git add -A
git commit -m "feat: Repla V2 complete — dual navbars, copilot, trading engine, positions panel"
```
