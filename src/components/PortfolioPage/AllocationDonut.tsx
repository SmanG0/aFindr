...
export default function AllocationDonut({ allocations, totalValue, size = 200 }: AllocationDonutProps) {
  ...
  return (
    <div style={{ position: "relative", width: outerW, height: outerH }}>
      ...
      {/* External labels — collision-avoidant, fade in staggered */}
      {(() => {
        const labelR = r + strokeWidth / 2 + 24;
        const MIN_GAP = 16; // minimum px between label centers

        // Build raw label positions
        const raw = arcs
          .filter((arc) => arc.pct >= 0.04 || arc.ticker === "Other")
          .map((arc) => {
        ...
        // Split into left/right halves, push apart within each half
        for (const side of ["left", "right"] as const) {
          const group = raw
            .filter((l) => (side === "right" ? l.lx >= centerX : l.lx < centerX))
            .sort((a, b) => a.ly - b.ly);
