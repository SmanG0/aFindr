...
interface PerformanceChartProps {
  ...
  height?: number;
  accentColor?: string;
  onHover?: (data: ChartHoverData | null) => void;
}
...
export default function PerformanceChart({
  ...
  // Benchmark is time-aligned: trimmed to match portfolio's time range,
  // then re-normalized so its % change starts from the same date as the portfolio.
  const { portNorm, benchNorm, minPct, maxPct } = useMemo(() => {
    const pStart = portfolioPoints[0]?.value || 1;
    const pNorm = portfolioPoints.length >= 2
      ? portfolioPoints.map((p) => ({
          time: p.time,
          pct: pStart !== 0 ? ((p.value - pStart) / pStart) * 100 : 0,
        }))
      : [];

    // Align benchmark to portfolio's time range
    let alignedBench = benchmarkPoints;
    if (portfolioPoints.length >= 2 && benchmarkPoints.length >= 2) {
      const portStart = portfolioPoints[0].time;
      const portEnd = portfolioPoints[portfolioPoints.length - 1].time;
      // Find benchmark points within portfolio's time window (with small tolerance)
      const tolerance = 86400000; // 1 day in ms
      const trimmed = benchmarkPoints.filter(
        (b) => b.time >= portStart - tolerance && b.time <= portEnd + tolerance
      );
      if (trimmed.length >= 2) {
        alignedBench = trimmed;
      }
    }

    const bStart = alignedBench[0]?.value || 1;
    const bNorm = alignedBench.length >= 2
      ? alignedBench.map((p) => ({
          time: p.time,
          pct: bStart !== 0 ? ((p.value - bStart) / bStart) * 100 : 0,
        }))
      : [];
  ...
  // Use a generous overestimate — with preserveAspectRatio="none", SVG
  // path length calculation can diverge from our viewBox-space estimate.
  const approxLineLen = useMemo(() => {
    ...
    // Add 20% buffer + flat padding to guarantee the dash covers the full path
    return len * 1.2 + 200;
  }, [portNorm, toY, W]);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
    ...
    // Pass actual value back to parent for hero update
    const hoveredPoint = portfolioPoints[clampedIdx];
    if (hoveredPoint && onHover) {
      const startVal = portfolioPoints[0].value;
      onHover({
        value: hoveredPoint.value,
        time: hoveredPoint.time,
        pctChange: portPct,
        dollarChange: hoveredPoint.value - startVal,
      });
    }
  }, [portNorm, benchNorm, portfolioPoints, W, onHover]);
