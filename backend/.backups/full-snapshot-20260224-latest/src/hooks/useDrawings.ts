...
export function useDrawings() {
  ...
  const convexDrawings = useQuery(api.charts.getDrawings, isAuthenticated ? { symbol: "__all__" } : "skip");
  const saveConvexDrawings = useMutation(api.charts.saveDrawings);
  ...
  useEffect(() => {
    if (!hydrated.current || !isAuthenticated || convexDrawings === undefined || convexReconciledRef.current) return;
    convexReconciledRef.current = true;
    if (convexDrawings && convexDrawings.drawingsJson) {
      try {
        const parsed = JSON.parse(convexDrawings.drawingsJson) as Record<string, unknown>[];
        if (parsed.length > 0) {
          const migrated = parsed.map(migrateDrawing);
          setDrawings(migrated);
          saveDrawings(migrated);
          return;
        }
      } catch { /* ignore corrupt data */ }
    }
    // Convex empty → seed from localStorage
    const current = loadDrawings();
    if (current.length > 0) {
      saveConvexDrawings({ symbol: "__all__", drawingsJson: JSON.stringify(current) });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAuthenticated, convexDrawings]);

  // Persist whenever drawings change (skip initial hydration)
  // localStorage is immediate, Convex is debounced by 1 second
  const convexDebounceRef = useRef<NodeJS.Timeout | null>(null);
  useEffect(() => {
    if (hydrated.current) {
      saveDrawings(drawings);
      // Debounced Convex write
      if (isAuthenticated) {
        if (convexDebounceRef.current) clearTimeout(convexDebounceRef.current);
        convexDebounceRef.current = setTimeout(() => {
          saveConvexDrawings({ symbol: "__all__", drawingsJson: JSON.stringify(drawings) });
        }, 1000);
      }
    }
    return () => {
