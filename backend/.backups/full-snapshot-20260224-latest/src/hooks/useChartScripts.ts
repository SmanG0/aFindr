...
export function useChartScripts(candles: Candle[], currentSymbol?: string) {
  const [scripts, setScripts] = useState<ChartScript[]>([]);

  // ─── Convex sync ───
  const { isAuthenticated } = useCurrentUser();
  const convexScripts = useQuery(api.charts.listScripts, isAuthenticated ? {} : "skip");
  ...
  useEffect(() => {
    if (!isAuthenticated || convexScripts === undefined || convexReconciledRef.current) return;
  ...
  // Evaluate visible scripts against candles (filtered by current symbol)
  const scriptResults: ChartScriptResult[] = useMemo(() => {
    return scripts
      .filter((s) => s.visible)
      .filter((s) => !s.symbol || !currentSymbol || s.symbol === currentSymbol)
      .map((s) => evaluateChartScript(s, candles));
  }, [scripts, candles, currentSymbol]);
  ...
  const addScript = useCallback((script: ChartScript) => {
    setScripts((prev) => {
      ...
      // Deduplicate by generator types within the SAME symbol: if new script has
      // generators matching an existing script's generators for the same symbol,
      // replace the old one (handles color/style updates).
      if (script.generators.length > 0) {
        ...
        const matchIdx = prev.findIndex((s) =>
          ...
          (s.symbol ?? "") === (script.symbol ?? "") // Only dedupe within same symbol
        );
        if (matchIdx >= 0) {
          const oldId = prev[matchIdx].id;
          if (oldId !== script.id && isAuthenticated) {
            removeScriptMut({ scriptId: oldId });
          }
          const next = [...prev];
      ...
      // Deduplicate by name within the SAME symbol
      const nameMatchIdx = prev.findIndex((s) =>
        s.name === script.name &&
        (s.symbol ?? "") === (script.symbol ?? "")
      );
      if (nameMatchIdx >= 0) {
  ...
  }, [isAuthenticated, upsertScriptMut, removeScriptMut]);

  const removeScript = useCallback((id: string) => {
    setScripts((prev) => prev.filter((s) => s.id !== id));
    if (isAuthenticated) {
      removeScriptMut({ scriptId: id });
    }
  }, [isAuthenticated, removeScriptMut]);
  ...
  const toggleScript = useCallback((id: string) => {
    setScripts((prev) => {
      ...
      if (isAuthenticated && toggled) {
        upsertScriptMut({
          scriptId: toggled.id,
          name: toggled.name,
          symbol: toggled.symbol,
  ...
  }, [isAuthenticated, upsertScriptMut]);

  const clearAllScripts = useCallback(() => {
    // Remove each from Convex
    if (isAuthenticated) {
      for (const s of scripts) {
        removeScriptMut({ scriptId: s.id });
      }
    }
    setScripts([]);
  }, [isAuthenticated, scripts, removeScriptMut]);
  ...
  const deleteScriptByName = useCallback((name: string) => {
    setScripts(prev => {
      ...
      if (match) {
        if (isAuthenticated) removeScriptMut({ scriptId: match.id });
