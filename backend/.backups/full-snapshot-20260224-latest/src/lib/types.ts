...
// Tool data entry for rich artifact rendering (options chains, earnings, etc.)
...
export interface ChatMessage {
  ...
  // Rich tool data for artifact rendering (options, earnings, news blocks, etc.)
  toolData?: ToolDataEntry[];
  // NOTE: Added for Agent SDK SSE streaming — true while tokens are arriving
  isStreaming?: boolean;
  // Per-message token usage (premium feature)
  tokenUsage?: {
    totalTokens: number;
    inputTokens: number;
    outputTokens: number;
