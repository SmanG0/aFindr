...
export default function Home() {
  ...
  // ═══════════════════════════════════════════════
  const VALID_PAGES = new Set<AppPage>(["trade", "dashboard", "portfolio", "news", "alpha", "settings", "journal", "library"]);
  ...
  const handleSubmitStreaming = useCallback(
    async (message: string) => {
      ...
      try {
        ...
        // Load user profile for agent personalization
        let userProfile: { name: string; experience: string; tradingStyle: string; analysisApproach: string[]; tradingGoals: string[]; markets: string[] } | undefined;
        try {
          const profile = JSON.parse(localStorage.getItem("afindr_onboarding") || "{}");
