...
function EconomicDataBlock({ data }: { data: EconomicDataPayload }) {
  ...
  // Build wider sparkline SVG
  const sparkW = 220, sparkH = 48;
  const sparkPoints = values.length >= 2
    ? values.map((v, i) => {
        const min = Math.min(...values);
        const range = Math.max(...values) - min || 1;
        return `${(i / (values.length - 1)) * sparkW},${sparkH - 3 - ((v - min) / range) * (sparkH - 6)}`;
      }).join(" ")
    : "";
  ...
  return (
    <BlockCollapse label={data.info?.title || data.seriesId || "Economic Data"}>
      <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
        {/* Hero row: metric left, sparkline right */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          {/* Wider sparkline */}
          {values.length >= 3 && (
            <svg width={sparkW} height={sparkH} viewBox={`0 0 ${sparkW} ${sparkH}`} style={{ display: "block", flexShrink: 0 }}>
              <defs>
              ...
              />
              <polyline points={sparkPoints} fill="none" stroke={accentColor} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          )}
        </div>
