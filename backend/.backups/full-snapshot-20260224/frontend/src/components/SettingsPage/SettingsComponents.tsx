  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", gap: 12 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {subtitle && <span style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2, display: "block" }}>{subtitle}</span>}
      </div>
      {children}
    </div>
  );
...
export function SelectDropdown({ value, options, onChange }: { value: string; options: { id: string; label: string }[]; onChange: (v: string) => void }) {
  return (
    <select
      value={value}
  ...
  return (
    <input
      ...
      style={{
        width: "100%", background: "rgba(236,227,213,0.04)",
        border: "1px solid rgba(236,227,213,0.08)", borderRadius: 8,
        color: "var(--text-primary)", fontSize: 13, padding: "10px 14px", outline: "none",
      }}
      onFocus={(e) => { e.currentTarget.style.borderColor = "var(--accent)"; }}
      onBlur={(e) => { e.currentTarget.style.borderColor = "rgba(236,227,213,0.08)"; }}
    />
