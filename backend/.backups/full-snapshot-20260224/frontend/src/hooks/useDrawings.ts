...
export function useDrawings() {
  ...
  const convexDrawings = useQuery(api.charts.getDrawings, userId ? { userId, symbol: "__all__" } : "skip");
  ...
  useEffect(() => {
    if (!hydrated.current || !userId || convexDrawings === undefined || convexReconciledRef.current) return;
    ...
    if (current.length > 0) {
      saveConvexDrawings({ userId, symbol: "__all__", drawingsJson: JSON.stringify(current) });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userId, convexDrawings]);
    ...
    if (hydrated.current) {
      ...
      if (userId) {
        ...
        convexDebounceRef.current = setTimeout(() => {
          saveConvexDrawings({ userId, symbol: "__all__", drawingsJson: JSON.stringify(drawings) });
        }, 1000);
      }
    }
    return () => {
