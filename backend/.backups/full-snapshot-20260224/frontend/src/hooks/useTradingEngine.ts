...
export function useTradingEngine(userId: Id<"users"> | null) {
  ...
  useEffect(() => {
    if (!stateHydrated || !userId || convexSyncedRef.current) return;
  ...
  const resetAccount = useCallback(() => {
    ...
    // Sync reset state to Convex
    if (userId) {
      syncFullStateMut({ userId, positions: [], trades: [] });
    }
  }, [userId, syncFullStateMut]);
