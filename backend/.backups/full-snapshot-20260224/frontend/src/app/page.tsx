...
function parseIndicatorTags(message: string): {
  ...
  const updateRegex = /\[SCRIPT_UPDATE:([^:]+):([^\]]+)\]/g;
...
export default function Home() {
  ...
  const convexConversations = useQuery(api.chat.listConversations, userId ? { userId } : "skip");
  ...
  useEffect(() => {
    if (!userId || chatRestoredRef.current || convexConversations === undefined) return;
    chatRestoredRef.current = true;
    if (convexConversations.length > 0) {
      setActiveConversationId(convexConversations[0]._id);
    }
  }, [userId, convexConversations]);

  // When we have an active conversation, restore messages from Convex
  const messagesRestoredRef = useRef(false);
  useEffect(() => {
    if (!activeConversationId || convexMessages === undefined || messagesRestoredRef.current) return;
    if (convexMessages.length > 0 && messages.length === 0) {
  ...
  // ═══════════════════════════════════════════════
  const tradingEngine = useTradingEngine(userId);
  ...
  const convexSettings = useQuery(api.settings.getSettings, userId ? { userId } : "skip");
  ...
  useEffect(() => {
    if (!userId || convexSettings === undefined || settingsReconciledRef.current) return;
    settingsReconciledRef.current = true;
    if (convexSettings) {
      // Convex has data → use it, update localStorage
      const { _id, _creationTime, userId: _u, updatedAt: _t, ...rest } = convexSettings;
      ...
      upsertSettings({ userId, settings: appSettings });
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userId, convexSettings]);
  ...
  const setAppSettings = useCallback((next: AppSettings | ((prev: AppSettings) => AppSettings)) => {
    setAppSettingsState((prev) => {
      ...
      if (userId) {
        upsertSettings({ userId, settings: nextVal });
      }
      return nextVal;
    });
  }, [userId, upsertSettings]);
  ...
  const {
    ...
    removeScript: removeChartScript,
  ...
  }, [userId, createConversation, addChatMessage]);
  ...
  const handleSubmit = useCallback(
    async (message: string) => {
      ...
      try {
        ...
        const chartScriptsArr = (response as any).chart_scripts ?? (response as any).chartScripts;
        ...
        } else if ((response as any).chart_script ?? (response as any).chartScript) {
          parsedChartScript = ((response as any).chart_script ?? (response as any).chartScript) as ChartScript;
  ...
  const handleSubmitStreaming = useCallback(
    async (message: string) => {
      ...
      try {
        ...
        const doneEvent = await agentStream.streamMessage({
          ...
          active_scripts: chartScripts.filter(s => s.visible).map(s => s.name),
        });
    ...
    [messages, symbol, interval, addChartScript, handleSetAlphyIndicators, clearAllChartScripts, updateScriptByName, deleteScriptByName, handleSubmit, agentStream, persistChatMessage]
  );
