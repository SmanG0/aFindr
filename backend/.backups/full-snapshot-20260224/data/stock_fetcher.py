...
def fetch_stock_quote(ticker: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Fetch intraday price data for mini chart (1d at 5m intervals)
        intraday = []
        open_price = _safe_float(info.get("regularMarketOpen")) or prev_close
        try:
        ...
        data = {
            ...
            "volume": _format_large_count(info.get("regularMarketVolume")),
            "avgVolume": _format_large_count(info.get("averageDailyVolume10Day")),
            "pe": str(round(info.get("trailingPE", 0), 2)) if info.get("trailingPE") else None,
            "eps": str(round(info.get("trailingEps", 0), 2)) if info.get("trailingEps") else None,
            "divYield": f"{round(info.get('dividendYield', 0) * 100, 2)}%" if info.get("dividendYield") else None,
            "beta": str(round(info.get("beta", 0), 2)) if info.get("beta") else None,
